...
public class ThemeAndCostChosenHandler implements GameMessageHandler {
    ...
    public void handle(AbstractMessage message, Game game) {
        ...
        final int questionNumber = Math.abs(new SecureRandom().nextInt()) % countOfPossibleQuestions;
        ...
        try {
            ...
            QuestionMessage questionMessage = new QuestionMessage(questionModel,playerConverter.convert(new ArrayList<Player>(game.getPlayers().values())));
            ...
            final QuestionModel questionModel1 = questionModel;
                    ...
                    try {
                     Thread.sleep(question.getTimeForAnswer());
                        if (this == game1.getEverybodyAnsweredChecker()) {
                         AnsweringFinishedMessage answeringFinishedMessage = new AnsweringFinishedMessage(webSocketSession,questionModel1.getCorrectAnswer());
